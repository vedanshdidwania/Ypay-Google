...
export default function Sell() {
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      const inrAmount = parseFloat(usdtAmount) * (settings?.sell_rate || 1);
      ...
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl">
        ...
        {step === 1 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            <h2 className="text-2xl font-bold mb-6">Enter USDT Amount</h2>
            <div className="space-y-6">
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="0.00"
                  />
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 font-bold text-blue-400">USDT</div>
                </div>
              </div>
              <div className="p-4 rounded-2xl bg-green-500/5 border border-green-500/10 flex items-center space-x-4">
                ...
                <div>
                  <div className="text-sm font-bold">You will receive</div>
                  <div className="text-xl font-bold text-green-400">
                    {formatCurrency(parseFloat(usdtAmount || '0') * (settings?.sell_rate || 1))}
                  </div>
                </div>
              </div>
              <button
        ...
        {step === 2 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            <h2 className="text-2xl font-bold mb-2">Transfer USDT</h2>
            <p className="text-gray-400 mb-8">Send USDT (TRC20) to the wallet address below.</p>

            <div className="bg-black/40 border border-white/10 rounded-2xl p-6 mb-8 space-y-6">
              <div className="flex justify-between items-center pb-4 border-b border-white/5">
                <span className="text-gray-500 font-medium">Amount to Send</span>
                <span className="text-2xl font-bold text-blue-400">{formatUSDT(parseFloat(usdtAmount))}</span>
              </div>
              
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Our USDT Wallet (TRC20)</label>
                <div className="flex items-center space-x-2">
                  <div className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 font-mono text-sm truncate">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500/50 transition-colors font-mono text-sm"
